...
include('../../bd_final.php');
	...
	if((isset($_POST['id_utc'])) && (isset($_POST['semestre_atual'])) && (isset($_POST['id_docente']))){	
		...
		$statement = mysqli_prepare($conn, "SELECT DISTINCT a.*, t.semestre, c.id_curso FROM aula a INNER JOIN turma t ON a.id_turma = t.id_turma INNER JOIN curso c ON t.id_curso = c.id_curso WHERE c.id_utc = $id_utc AND t.semestre = $semestre_atual AND a.id_docente = $id_docente AND a.id_horario IS NULL;");
		$statement->execute();
