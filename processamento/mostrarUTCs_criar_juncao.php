...
include('../bd_final.php');
	...
	if(!isset($_POST["array_turmas_temp"])){
		...
		$statement0 = mysqli_prepare($conn, "SELECT DISTINCT id_utc, nome_utc FROM utc ORDER BY id_utc;");
		$statement0->execute();
		$resultado0 = $statement0->get_result();
		while($linha0 = mysqli_fetch_array($resultado0)){
			$id_utc = $linha0['id_utc'];
			$nome_utc = $linha0['nome_utc'];
							...
							while($linha3 = mysqli_fetch_assoc($resultado3)){
								...
								while($linha4 = mysqli_fetch_assoc($resultado4)){
									...
									$linha5 = mysqli_fetch_assoc($resultado5);
										...
										if($count_juncao == 0){
											if(!in_array($id_utc,$utcs)){
												array_push($utcs, $id_utc);
												array_push($utcs, $nome_utc);
											}
										}
								}
							}
		...
		$statement02 = mysqli_prepare($conn, "SELECT DISTINCT id_utc, nome_utc FROM utc ORDER BY id_utc;");
		$statement02->execute();
		$resultado02 = $statement02->get_result();
		while($linha02 = mysqli_fetch_array($resultado02)){
			$id_utc = $linha02['id_utc'];
			$nome_utc = $linha02['nome_utc'];
