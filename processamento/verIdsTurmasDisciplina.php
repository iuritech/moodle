...
include('../bd_final.php');
	...
	if(isset($_POST['uc'], $_POST['curso'])){
		...
		while($linha2 = mysqli_fetch_array($resultado2)){
			...
			array_push($componentes_ids, $id_componente);
			
		}
		
		$componentes_final = implode(",",$componentes_ids);
		
			//Todas as turmas que pertencem a esses blocos a ao curso escolhido
			$statement4 = mysqli_prepare($conn, "SELECT DISTINCT t.id_turma from Turma t LEFT JOIN componente_turma ct ON t.id_turma = ct.id_turma WHERE ct.id_componente IN ($componentes_final) && t.id_curso = $id_curso;");
			$statement4->execute();
			$resultado4 = $statement4->get_result();
			while($linha4 = mysqli_fetch_assoc($resultado4)){
				array_push($turmas_ids, (int) $linha4['id_turma']);
			}	
		
		$List = implode(', ', $turmas_ids);
		print_r($List);
