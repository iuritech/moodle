...
if(isset($_POST['id_componente']) && isset($_POST['array_turmas']) && isset($_POST['id_docente']) && isset($_POST['id_juncao'])){
	...
	//Juntar as turmas escolhidas à junção
	$j = 0;
	...
	if($id_docente != 0){
		...
		//Atribuir o docente que estava na junção às outras turmas
		$j = 0;
		while($j < sizeof($array_turmas)){
			$id_turma = $array_turmas[$j];
			$id_componente = $array_turmas[$j + 1];
