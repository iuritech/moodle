function gerarFormCriarUtil() {
    var xhttp;    
    
    xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        document.getElementById("modalBodyCriarUtil").innerHTML = this.responseText;
      }
    };
    xhttp.open("GET", "phpUtil/formCriarUtil.php", true);
    xhttp.send();
}

function gerarFormEditarUtil(id) {
    var xhttp;    
    if (id == "") {
      document.getElementById("modalBodyEditarUtil").innerHTML = "";
      return;
    }
    xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        document.getElementById("modalBodyEditarUtil").innerHTML = this.responseText;
      }
    };
    xhttp.open("GET", "phpUtil/formEditarUtil.php?i="+id, true);
    xhttp.send();
}

function gerarFormRemoverUtil(id) {
    var xhttp;    
    if (id == "") {
      document.getElementById("modalBodyRemoverUtil").innerHTML = "";
      return;
    }
    xhttp = new XMLHttpRequest();
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        document.getElementById("modalBodyRemoverUtil").innerHTML = this.responseText;
      }
    };
    xhttp.open("GET", "phpUtil/formRemoverUtil.php?i="+id, true);
    xhttp.send();
}
